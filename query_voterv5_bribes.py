#!/usr/bin/env python3
"""
Query VoterV5 (which delegatecalls BribeV2) to find reward tokens
Since VoterV5 is a proxy, the BribeV2 code executes in VoterV5's storage
"""
...
# VoterV5 proxy address
voterv5_addr = w3.to_checksum_address("0xc69E3eF39E3fFBcE2A1c570f8d3ADF76909ef17b")

print(f"Querying VoterV5 (proxy delegatecalling to BribeV2): {voterv5_addr}")
print("=" * 80)

# Use BribeV2 ABI to call VoterV5 (since it delegatecalls BribeV2 code)
contract = w3.eth.contract(address=voterv5_addr, abi=bribev2_abi)

try:
    # Get number of rewards
    rewards_count = contract.functions.rewardsListLength().call()
    print(f"\n✓ Total reward tokens registered in VoterV5: {rewards_count}")
    
    # Get each token address
    print(f"\nTokens found in VoterV5 storage:")
    print("-" * 80)
    ...
    print("SUMMARY")
    print("=" * 80)
    print(f"\nMissing tokens found in VoterV5:")
    ...
    if still_missing:
        print(f"\nStill missing:")
        for symbol in sorted(still_missing):
            print(f"  ❌ {symbol}: {missing_tokens[symbol]}")

except Exception as e:
    print(f"Error querying VoterV5: {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)

...
def print_voting_instructions(
    ...
    console.print(
        f"[bold cyan]║       HYDREX VOTING INSTRUCTIONS - PHASE 0.1          ║[/bold cyan]"
    )
    console.print(
    ...
    console.print(
        f"[yellow]Snapshot Timestamp:[/yellow] {snapshot_ts} ({time.strftime('%Y-%m-%d %H:%M:%S UTC', time.gmtime(snapshot_ts))})"
    )
    console.print(
        f"[yellow]Vote Epoch:[/yellow] {vote_epoch} ({time.strftime('%Y-%m-%d %H:%M:%S UTC', time.gmtime(vote_epoch))})"
    )
    ...
    # Allocation table
    table = Table(
        title="Recommended Allocation", show_header=True, header_style="bold magenta"
    )
    ...
    console.print(
        f"[bold green]Expected Total Return (normalized):[/bold green] {total_expected_return:,.2f}\n"
    )

    # Contract call format
    console.print("[bold cyan]═══ CONTRACT CALL FORMAT ═══[/bold cyan]\n")
    console.print(
        "[yellow]Function:[/yellow] vote(address[] _poolVote, uint256[] _voteProportions)"
    )
    console.print(
        f"[yellow]Contract:[/yellow] {os.getenv('VOTER_ADDRESS', 'VOTER_ADDRESS_NOT_SET')}\n"
    )
    ...
    console.print(
        f"vote_proportions = [{PROPORTION_PER_POOL}] * {len(allocation)}  # Equal weights"
    )
    console.print("# OR: vote_proportions = [10000, 10000, ...]  # One per pool")
    console.print(
        "tx = voter_contract.functions.vote(pool_addresses, vote_proportions).build_transaction({...})"
    )
    console.print("```")
    ...
    if output_csv:
        with open(output_csv, "w") as f:
            f.write(
                "rank,gauge_address,pool_address,base_votes,rewards_normalized,marginal_roi,allocated_votes,percent_of_power\n"
            )
            for idx, (
                gauge_addr,
                pool_addr,
                ...
                pct = (allocated_votes / your_voting_power) * 100.0
                f.write(
                    f"{idx},{gauge_addr},{pool_addr},{base_votes:.2f},{rewards_norm:.4f},{adj_roi:.8f},{allocated_votes},{pct:.2f}\n"
                )
    ...
    console.print(
        "[red]1. Verify pool addresses match gauges in your wallet/interface[/red]"
    )
    ...
    console.print(
        "[red]4. This is an EQUAL ALLOCATION strategy (may not be optimal)[/red]"
    )
    console.print(
        "[red]5. Review transaction in block explorer before confirming[/red]\n"
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate voting instructions from latest snapshot"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Minimum reward threshold (normalized)",
    )
    parser.add_argument(
        "--output-csv", default="", help="Optional CSV output file path"
    )
    parser.add_argument(
        "--snapshot-ts",
        type=int,
        default=0,
        help="Specific snapshot timestamp (default: latest)",
    )
    parser.add_argument(
        "--save-prediction",
        action="store_true",
    ...
    if args.your_voting_power <= 0:
        console.print(
            "[red]Error: YOUR_VOTING_POWER must be > 0 (set in .env or pass --your-voting-power)[/red]"
        )
        sys.exit(1)

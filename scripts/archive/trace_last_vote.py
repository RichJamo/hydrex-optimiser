...
summary_table = Table(
    title="Vote → Gauge → Bribe Contracts → Payments", show_header=True, show_lines=True
)
...
for pool in pools_voted:
    ...
    if result:
        ...
        summary_table.add_row(
            f"{pool[:8]}...{pool[-6:]}", "[red]NOT FOUND[/red]", "", "", ""
        )
...
for payment in payments:
    ...
    pool_display = (
        f"{matched_pool[:8]}...{matched_pool[-6:]}"
        if matched_pool
...
console.print(
    f"  Return per 1K Votes: [yellow]${(total_received / 1183.272):.2f}[/yellow]"
)

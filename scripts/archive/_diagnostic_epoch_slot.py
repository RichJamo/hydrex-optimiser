...
minter = w3.eth.contract(
    address=Web3.to_checksum_address(MINTER_ADDRESS), abi=MINTER_ABI
)
...
for label, block in [
    ...
    write_key = ap + WEEK
    print(
        f"{label} (block {block}): active_period={ap}  write_key={write_key}  vote_epoch_slot_frozen={ap >= VOTE_EPOCH}"
    )
...
if row:
    ...
    bribe = w3.eth.contract(
        address=Web3.to_checksum_address(TOP_POOL_BRIBE), abi=BRIBE_ABI
    )
    for label, block in [
        ("live block", LIVE_BLOCK),
        ("boundary block", BOUNDARY_BLOCK),
    ]:
        rd = bribe.functions.rewardData(
            Web3.to_checksum_address(TOP_POOL_TOKEN), VOTE_EPOCH
        ).call(block_identifier=block)
        print(
            f"  {label}: periodFinish={rd[0]}  rewardsPerEpoch={rd[1]}  lastUpdateTime={rd[2]}"
        )

...
if "current_votes" in columns:
    ...
    if current_type != "TEXT":
        ...
        cursor.execute(
            "UPDATE gauges SET current_votes_text = CAST(current_votes AS TEXT)"
        )
        ...
        cursor.execute(
            "SELECT address, pool, internal_bribe, external_bribe, is_alive, created_at, last_updated, current_votes_text FROM gauges"
        )

...
class VotePerformanceAnalyzer:
    ...
    def __init__(self):
        ...
        self.voter = self.w3.eth.contract(
            address=Web3.to_checksum_address(Config.VOTER_ADDRESS), abi=self.voter_abi
        )
    ...
    def analyze_last_vote(
        self, pools_voted: List[str], rewards_received: Dict[str, float]
    ) -> Dict:
        """
        ...
        for pool, reward in sorted(
            rewards_received.items(), key=lambda x: x[1], reverse=True
        ):
        ...
        console.print(
            f"  Return per 1K Votes: [yellow]${reward_per_1k_votes:.3f}[/yellow]"
        )
    ...
    def calculate_expected_returns(
        self, gauge_data: List[Dict], allocation: Dict[str, int]
    ...
    def generate_recommendations(self, last_performance: Dict, gauge_data: List[Dict]):
        """
        ...
        strategies = [
            ...
            (
                "Optimized Multi-Gauge",
                multi_return,
                multi_return - last_return,
        ...
        for name, expected, diff, gauges in strategies:
            ...
            table.add_row(name, f"${expected:.2f}", diff_str, str(gauges))
        ...
        improvement = (
            ((best_return - last_return) / last_return * 100) if last_return > 0 else 0
        )
        ...
        console.print(
            f"  Improvement: [yellow]{improvement:+.1f}%[/yellow] (${best_return - last_return:+.2f})"
        )
        ...
        for i, gauge in enumerate(gauge_data[:10], 1):
            ratio = (
                gauge["bribes_usd"] / gauge["current_votes"]
                if gauge["current_votes"] > 0
                else float("inf")
            )
            opp_table.add_row(
                str(i),

...
console.print(
    f"[yellow]TEST MODE: Processing only {TEST_GAUGE_LIMIT} gauges[/yellow]\n"
)
...
def get_all_gauges():
    ...
    try:
        ...
        console.print(
            f"[yellow]TEST: Fetching only first {TEST_GAUGE_LIMIT} gauges[/yellow]"
        )
        ...
        for i in range(min(TEST_GAUGE_LIMIT, gauge_count)):
            try:
                ...
                console.print(
                    f"  {i+1}. Pool: {pool_addr[:10]}... → Gauge: {gauge_addr}"
                )

            except Exception as e:
                console.print(
                    f"[yellow]Warning: Error fetching pool/gauge {i}: {e}[/yellow]"
                )
...
def scan_bribes(gauge_details):
    ...
    for i, gauge in enumerate(gauge_details, 1):
        ...
        if (
            ...
            and gauge["internal_bribe"] != "0x0000000000000000000000000000000000000000"
        ):
            console.print(
                f"  Scanning internal bribe: {gauge['internal_bribe'][:20]}..."
            )
            rewards = get_bribe_rewards(
                gauge["internal_bribe"], "internal", gauge["address"]
            )
            if rewards:
                for r in rewards:
                    usd_str = f"${r['usd_value']:.2f}" if r["usd_value"] else "No price"
                    console.print(
                        f"    • {r['amount']:.4f} {r['token_symbol']} ({usd_str})"
                    )
            else:
        ...
        if (
            ...
            and gauge["external_bribe"] != "0x0000000000000000000000000000000000000000"
        ):
            console.print(
                f"  Scanning external bribe: {gauge['external_bribe'][:20]}..."
            )
            rewards = get_bribe_rewards(
                gauge["external_bribe"], "external", gauge["address"]
            )
            if rewards:
                for r in rewards:
                    usd_str = f"${r['usd_value']:.2f}" if r["usd_value"] else "No price"
...
def display_summary(gauge_details, all_rewards):
    ...
    # Calculate totals
    total_internal_usd = sum(
        r["usd_value"]
        for r in all_rewards
        if r["usd_value"] and r["bribe_type"] == "internal"
    )
    total_external_usd = sum(
        r["usd_value"]
        for r in all_rewards
        if r["usd_value"] and r["bribe_type"] == "external"
    )
    ...
    table.add_row(
        "  [bold]Total (USD)[/bold]",
        f"[bold]${total_internal_usd + total_external_usd:,.2f}[/bold]",
    )

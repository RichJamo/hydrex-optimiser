    ...
    try:
        ...
        with Progress(
            ...
            task = progress.add_task(
                f"Fetching {gauge_count} gauges...", total=gauge_count
            )
            ...
            for i in range(gauge_count):
                try:
                    ...
                    console.print(
                        f"[yellow]Warning: Error fetching gauge {i}: {e}[/yellow]"
                    )
                    progress.update(task, advance=1)
...
def store_gauges(gauge_details):
    ...
    for gauge in gauge_details:
        ...
        cursor.execute(
            "SELECT address FROM gauges WHERE address = ?", (gauge["address"],)
        )
    ...
    cursor.execute(
        "SELECT COUNT(*) FROM gauges WHERE current_votes IS NOT NULL AND current_votes != '0'"
    )
...
def get_current_epoch():
    ...
    epoch_start = now.replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(
    ...
    try:
        ...
        for i in range(reward_count):
            try:
                ...
                console.print(
                    f"[yellow]Warning: Error reading token {i} from {bribe_addr[:10]}...: {e}[/yellow]"
                )
                continue
...
def scan_all_bribes():
    ...
    console.print(
        f"Current epoch: {current_epoch} ({datetime.fromtimestamp(current_epoch)})"
    )
    ...
    with Progress(
        ...
        for gauge_addr, pool, internal_bribe, external_bribe in gauges:
            ...
            if (
                internal_bribe
                and internal_bribe != "0x0000000000000000000000000000000000000000"
            ):
            ...
            if (
                external_bribe
                and external_bribe != "0x0000000000000000000000000000000000000000"
            ):
    ...
    console.print(
        f"[green]✓ Found {len(all_rewards)} reward tokens across all bribes[/green]"
    )
...
def generate_summary():
    ...
    cursor.execute(
        "SELECT SUM(CAST(current_votes AS INTEGER)) FROM gauges WHERE current_votes IS NOT NULL"
    )
    total_votes = cursor.fetchone()[0] or 0

    # Bribe stats for current epoch
    cursor.execute(
        "SELECT COUNT(*) FROM bribes WHERE epoch = ? AND bribe_type = 'internal'",
        (current_epoch,),
    )
    internal_rewards = cursor.fetchone()[0]

    cursor.execute(
        "SELECT COUNT(*) FROM bribes WHERE epoch = ? AND bribe_type = 'external'",
        (current_epoch,),
    )
    ...
    table.add_row(
        "  [bold]Total Available Bribes (USD)[/bold]",
        f"[bold]${internal_usd + external_usd:,.2f}[/bold]",
    )
    table.add_row(
        "  Tokens with Prices", f"{total_rewards - missing_prices} / {total_rewards}"
    )
    if missing_prices > 0:
        table.add_row(
            "  [yellow]Tokens Missing Prices[/yellow]",
            f"[yellow]{missing_prices}[/yellow]",
        )

...
print(
    f"Current epoch timestamp : {epoch_ts}  ({time.strftime('%Y-%m-%d %H:%M UTC', time.gmtime(epoch_ts))})"
)
print(
    f"Next boundary           : {next_boundary}  ({time.strftime('%Y-%m-%d %H:%M UTC', time.gmtime(next_boundary))})"
)
print(
    f"Now (UTC)               : {now_utc}  ({time.strftime('%Y-%m-%d %H:%M UTC', time.gmtime(now_utc))})"
)
print(
    f"Seconds until boundary  : {secs_until:,}  ({secs_until // 3600}h {(secs_until % 3600) // 60}m {secs_until % 60}s)"
)

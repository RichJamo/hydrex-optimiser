...
for payment in payments:
    ...
    for vote in vote_data:
        if (
            vote.get("internal_bribe")
            and payment_contract == vote["internal_bribe"].lower()
        ):
            ...
            vote.get("external_bribe")
            and payment_contract == vote["external_bribe"].lower()
        ):
            payment["pool"] = vote["name"]
...
for vote in vote_data:
    ...
    for payment in payments:
        if (
            payment.get("pool_addr")
            and payment["pool_addr"].lower() == pool_addr.lower()
        ):
            token_list = ", ".join(
                [f"{t['token']}: ${t['usd']:.2f}" for t in payment["tokens"]]
            )
...
stats_table.add_row("", "")
stats_table.add_row(
    "[bold]Total Rewards Received[/bold]", f"[bold]${total_received:.2f}[/bold]"
)
...
for pool_name, reward in sorted_pools:
    ...
    perf_table.add_row(pool_name, f"${reward:.2f}", f"${per_1k:.2f}", f"{pct:.1f}%")

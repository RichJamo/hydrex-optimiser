...
def get_gauge_total_votes(epoch, gauges):
    ...
    for gauge_addr in gauges:
        ...
        print(
            f"[DEBUG] Query for gauge {gauge_addr[:16]}...: epoch={epoch}, gauge={gauge_addr}",
            flush=True,
        )
        ...
        try:
            ...
            if "errors" in result:
                print(
                    f"[DEBUG] ERROR for gauge {gauge_addr[:16]}...: {result['errors']}",
                    flush=True,
                )
                gauge_totals[gauge_addr] = 0
            elif (
                "data" in result and result["data"] and result["data"].get("gaugeVotes")
            ):
                total_weight = sum(
                    float(vote["weight"]) / 1e18
                    for vote in result["data"]["gaugeVotes"]
                )
                gauge_totals[gauge_addr] = total_weight
                if total_weight > 0:
                    print(
                        f"[DEBUG] Gauge {gauge_addr[:16]}...: {total_weight:,.2f} total votes",
                        flush=True,
                    )
            else:
                print(
                    f"[DEBUG] No votes found for gauge {gauge_addr[:16]}...", flush=True
                )
                gauge_totals[gauge_addr] = 0
        except Exception as e:
            print(
                f"[DEBUG] Error fetching votes for gauge {gauge_addr[:16]}...: {e}",
                flush=True,
            )
            gauge_totals[gauge_addr] = 0
...
print(
    f"Found {len(your_votes_by_epoch)} voting epochs with {len(all_votes)} total votes"
)
print()
...
for vote_epoch in sorted(your_votes_by_epoch.keys(), reverse=True):
    ...
    reward_epoch = (
        vote_epoch + 604800
    )  # Rewards earned in the next epoch (1 week later)
    ...
    gauge_total_votes = get_gauge_total_votes(
        bribe_epoch, your_gauges_this_epoch.keys()
    )

    print(f"\nVoting Epoch: {vote_epoch_date} (ts={vote_epoch})")
    print(f"Reward Epoch: {reward_epoch_date} (ts={reward_epoch})")
    print(
        f"Total votes cast: {total_votes_cast:,.2f} across {len(your_gauges_this_epoch)} gauges"
    )
    print(
        f"(Bribes submitted in {vote_epoch_date} → rewards earned in {reward_epoch_date})"
    )
    print()

    print(
        f"[DEBUG] Fetching bribes from database for epoch {bribe_epoch}...", flush=True
    )
    # Fetch bribes for this epoch
    bribes = session.query(Bribe).filter(Bribe.epoch == bribe_epoch).all()
    ...
    print(
        f"[DEBUG] Fetching prices for {len(set(b.reward_token for b in bribes))} unique tokens...",
        flush=True,
    )
    # Get prices for this epoch
    unique_tokens = list(set(b.reward_token for b in bribes))
    token_prices = price_feed.get_batch_prices_for_timestamp(
        unique_tokens, bribe_epoch, granularity="day"
    )
    ...
    print(
        f"[DEBUG] Calculating your returns from {len(your_gauges_this_epoch)} gauges...",
        flush=True,
    )
    epoch_returns = 0.0
    print(
        f"{'Gauge':<12} {'Your Votes':<18} {'DB Total':<18} {'Your Share':<12} {'Your Return':<15}"
    )
    print("-" * 105)
    ...
    for gauge in sorted(your_gauges_this_epoch.keys()):
        ...
        print(
            f"{gauge[:10]}... {your_votes:>17,.2f} {gauge_total:>17,.2f} {your_share:>10.2%} ${your_return:>13,.2f}"
        )
        ...
        if gauge in tokens_by_gauge:
            ...
            sorted_tokens = sorted(
                token_summary.items(), key=lambda x: x[1]["value"], reverse=True
            )

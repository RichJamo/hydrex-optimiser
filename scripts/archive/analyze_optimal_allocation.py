...
console.print(
    f"  Improvement: {((single_best['expected_return'] / 954.53) - 1) * 100:+.1f}%"
)
...
for i, g in enumerate(top_n, 1):
    ...
    if i <= 5:
        console.print(
            f"  {i}. {g['pool'][:30]}... → {our_votes:,} votes → ${pool_return:.2f}"
        )
...
best_strategy_return = max(single_best["expected_return"], total_expected)

console.print(f"\nBased on current bribe levels and vote distribution:")
console.print(f"  • Your last vote baseline: [bold]${954.53:.2f}[/bold]")
console.print(
    f"  • Best estimated return: [bold green]${best_strategy_return:,.2f}[/bold green]"
)
console.print(
    f"  • Potential improvement: [bold]+{((best_strategy_return / 954.53) - 1) * 100:.1f}%[/bold]"
)
...
console.print(
    f"  • Internal bribes (fees) will grow as trading continues until epoch flip"
)

...
console.print(
    f"[yellow]Voting power available: {epoch_data['user_voting_power']:,}[/yellow]"
)
...
total_predicted_if_only_voter = sum(
    data["total_usd"] * (data["your_bp"] / 10_000) for data in pool_rewards.values()
)

console.print(
    f"[cyan]Total bribes if you were only voter: ${total_predicted_if_only_voter:,.2f}[/cyan]"
)
console.print(f"[cyan]Total rewards actually received: ${TOTAL_ACTUAL_USD:,.2f}[/cyan]")

dilution_factor = (
    total_predicted_if_only_voter / TOTAL_ACTUAL_USD if TOTAL_ACTUAL_USD > 0 else 0
)

console.print(f"\n[bold red]Vote dilution factor: {dilution_factor:.1f}x[/bold red]")
console.print(
    f"[red]Other votes captured {(dilution_factor - 1) * 100:.0f}% of the rewards you would have gotten[/red]"
)
...
for name, data in pool_rewards.items():
    ...
    if actual_reward > 0 and total_pool_bribes > 0:
        ...
        implied_total_votes = (
            (your_votes_for_pool / actual_share_pct) * 100
            if actual_share_pct > 0
            else 0
        )

...
def query_bribe_rewards(w3, bribe_address):
    ...
    if (
        not bribe_address
        or bribe_address.lower() == "0x0000000000000000000000000000000000000000"
    ):
        return None

    try:
        bribe = w3.eth.contract(
            address=Web3.to_checksum_address(bribe_address), abi=BRIBEV2_ABI
        )
        ...
        return {"contract": bribe_address, "token_count": length, "tokens": tokens}
...
def main():
            ...
            if internal_result:
                ...
                for token_addr in internal_result["tokens"]:
                    ...
                    if (
                        ...
                        not in all_tokens_found[token_addr]["contracts"]
                    ):
                        all_tokens_found[token_addr]["contracts"].append(
                            gauge.internal_bribe
                        )
    ...
    print(
        f"Total unique token addresses found across all contracts: {len(all_tokens_found)}"
    )
    print()

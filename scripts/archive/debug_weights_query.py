    ...
    try:
        ...
        if votes > 0:
            pools_with_votes.append(
                {"index": i, "pool": pool_addr, "gauge": gauge_addr, "votes": votes}
            )

        # Progress indicator
        if (i + 1) % 50 == 0:
            print(
                f"Checked {i + 1}/{total_pools} pools... (found {len(pools_with_votes)} with votes)"
            )
...
print(f"\n" + "=" * 80)
print(
    f"RESULTS: Found {len(pools_with_votes)} pools with votes out of {pools_checked} pools with gauges"
)
print("=" * 80)
...
if pools_with_votes:
    ...
    for rank, p in enumerate(sorted_pools[:20], 1):
        ...
        print(
            f"{rank:2d}. Pool #{p['index']:3d}: {p['votes']:>25,} votes ({pct:>6.2f}%)"
        )
        print(f"    Pool:  {p['pool']}")

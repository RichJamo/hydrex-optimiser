...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional epoch to run completeness checks",
    )
    parser.add_argument(
        "--vote-epoch",
        type=int,
        default=None,
        help="Vote epoch used for truth-label sync/check",
    )
    parser.add_argument(
        "--sync-truth-labels",
        action="store_true",
    ...
    if args.check_epoch is not None:
        ...
        table.add_row(
            "Recommendations complete", str(summary["recommendations_complete"])
        )
        table.add_row("Epoch complete", str(summary["epoch_complete"]))
        table.add_row(
            "Snapshot windows", ", ".join(summary["snapshots_windows_present"]) or "-"
        )
        table.add_row(
            "Forecast windows", ", ".join(summary["forecasts_windows_present"]) or "-"
        )
        table.add_row(
            "Recommendation windows",
            ", ".join(summary["recommendations_windows_present"]) or "-",
        )
        table.add_row(
            "Forecast scenario counts",
            json.dumps(summary["forecast_scenario_counts"], sort_keys=True),
        )
        ...
        if args.vote_epoch is not None and args.sync_truth_labels:
            inserted = upsert_truth_labels_from_boundary(
                conn, int(args.check_epoch), int(args.vote_epoch), active_only=1
            )
            coverage = get_truth_label_coverage(
                conn, int(args.check_epoch), int(args.vote_epoch)
            )
            ...
            truth_table.add_row(
                "Boundary gauge rows", str(coverage["boundary_gauge_rows"])
            )

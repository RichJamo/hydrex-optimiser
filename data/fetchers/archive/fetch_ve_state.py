...
def fetch_ve_state(
    ...
    """
    console.print(
        f"[cyan]Fetching ve state for tokenId {token_id} at epoch {epoch}[/cyan]"
    )
    ...
    def call_with_fallback(func, default_value, label):
        try:
            ...
            try:
                ...
                console.print(
                    f"[yellow]Using default {label}: {default_value}[/yellow]"
                )
...
def main():
    ...
    parser.add_argument(
        "--token-id", type=int, required=True, help="ve NFT token ID to query"
    )
    parser.add_argument(
        "--block",
        type=int,

...
def get_token_decimals_and_symbol(w3: Web3, token_address: str) -> Tuple[int, str]:
    ...
    try:
        ...
        console.print(
            f"[yellow]Warning: Could not fetch decimals/symbol for {token_address}: {e}[/yellow]"
        )
    ...
    for bribe_addr in track(bribe_contracts, description="Querying bribe contracts"):
        try:
            bribe_contract = w3.eth.contract(
                address=Web3.to_checksum_address(bribe_addr), abi=BRIBE_CALC_ABI
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fetch bribe rewards from on-chain contracts"
    )
    parser.add_argument("--epoch", type=int, required=True, help="Epoch timestamp")
    parser.add_argument(
        "--bribe-contracts",
        type=str,
        help="Comma-separated bribe contract addresses (if not provided, queries from DB)",
    )
    parser.add_argument(
    ...
    if args.repair_token_metadata:
        ...
        if not repair_script.exists():
            console.print(
                "[yellow]Token metadata repair script not found; skipping repair step.[/yellow]"
            )
            return

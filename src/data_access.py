...
class EpochBribesSummary:
    ...
    epoch: int
    total_bribes_amount: (
        float  # Total in human-readable units (with conversion applied)
    )
    bribe_count: int
    unique_tokens: int
...
class DataAccess:
    ...
    def get_bribes_for_epoch_detailed(
        ...
        gauge_by_bribe: Dict[str, Tuple[str, str, str]] = (
            {}
        )  # bribe_contract -> (gauge, pool, bribe_type)

        for gauge in all_gauges:
            if gauge.internal_bribe:
                gauge_by_bribe[gauge.internal_bribe.lower()] = (
                    gauge.address,
                    gauge.pool or "",
                    "internal",
                )
            if gauge.external_bribe:
                gauge_by_bribe[gauge.external_bribe.lower()] = (
                    gauge.address,
                    gauge.pool or "",
                    "external",
                )
        ...
        for bribe in bribes:
            ...
            if (
                pool_addresses
                and pool_name
                and pool_name.lower() not in [p.lower() for p in pool_addresses]
            ):
                continue

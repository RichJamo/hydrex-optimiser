...
def load_gauge_snapshots(
    conn: sqlite3.Connection, epoch: int, blocks_before: int, min_votes: float = 1000.0
) -> List[GaugeSnapshot]:
...
def optimize_portfolio(
    snapshots: List[GaugeSnapshot], voting_power: float = 1_183_272, top_n: int = 5
) -> OptimizationResult:
...
def calculate_locked_in_return(
    prediction: OptimizationResult, actual_snapshots: List[GaugeSnapshot]
) -> float:
...
def compare_predictions(
    conn: sqlite3.Connection, epoch: int, voting_power: float = 1_183_272
    ...
    pre1_roi_error = (
        abs(pre1_opt.expected_roi - actual_opt.expected_roi)
        / actual_opt.expected_roi
        * 100
        if actual_opt.expected_roi > 0
        else 0
    )
    pre20_roi_error = (
        abs(pre20_opt.expected_roi - actual_opt.expected_roi)
        / actual_opt.expected_roi
        * 100
        if actual_opt.expected_roi > 0
        else 0
    ...
    pre1_loss_pct = (
        (actual_opt.total_expected_return - pre1_locked_return)
        / actual_opt.total_expected_return
        * 100
        if actual_opt.total_expected_return > 0
        else 0
    )
    pre20_loss_pct = (
        (actual_opt.total_expected_return - pre20_locked_return)
        / actual_opt.total_expected_return
        * 100
        if actual_opt.total_expected_return > 0
        else 0
...
def analyze_all_epochs(
    ...
    for epoch in epochs:
        ...
        console.print(
            f"  Actual optimal return: {actual.total_expected_return / 1e18:.2f} tokens"
        )
        console.print(
            f"  Actual optimal ROI: {actual.expected_roi / 1e18:.6f} tokens/vote"
        )

        console.print(f"  [yellow]Pre-1 block:[/yellow]")
        console.print(f"    Top-5 overlap: {pre1.overlap_count}/5")
        console.print(
            f"    Locked-in return: {pre1.return_if_locked_in / 1e18:.2f} tokens ({pre1.return_loss_pct:.2f}% loss)"
        )

        console.print(f"  [yellow]Pre-20 blocks:[/yellow]")
        console.print(f"    Top-5 overlap: {pre20.overlap_count}/5")
        console.print(
            f"    Locked-in return: {pre20.return_if_locked_in / 1e18:.2f} tokens ({pre20.return_loss_pct:.2f}% loss)"
        )
        console.print()
    ...
    if pre1_overlaps:
        ...
        table.add_row(
            "Max Return Loss %", f"{max(pre1_losses):.2f}%", f"{max(pre20_losses):.2f}%"
        )

        table.add_row(
            "Min Return Loss %", f"{min(pre1_losses):.2f}%", f"{min(pre20_losses):.2f}%"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze pre-boundary prediction accuracy"
    )
    parser.add_argument(
        "--db-path", default=DATABASE_PATH, help="Path to SQLite database"
    )
    parser.add_argument(
        "--voting-power", type=float, default=1_183_272, help="Your voting power"
    )
    ...
    try:
        if args.epoch:
            ...
            if comparison:
                ...
                console.print(
                    f"  Expected return: {actual.total_expected_return / 1e18:.2f} tokens"
                )
                console.print(
                    f"  Average ROI: {actual.expected_roi / 1e18:.6f} tokens/vote\n"
                )
                ...
                console.print(
                    f"  Predicted return: {pre1.prediction.total_expected_return / 1e18:.2f} tokens"
                )
                console.print(
                    f"  Actual return if locked in: {pre1.return_if_locked_in / 1e18:.2f} tokens"
                )
                ...
                console.print(
                    f"  Predicted return: {pre20.prediction.total_expected_return / 1e18:.2f} tokens"
                )
                console.print(
                    f"  Actual return if locked in: {pre20.return_if_locked_in / 1e18:.2f} tokens"
                )

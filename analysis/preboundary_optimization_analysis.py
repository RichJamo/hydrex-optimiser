...
class PreBoundaryAnalyzer:
    ...
    def get_preboundary_data(self, epoch: int, blocks_before: int) -> pd.DataFrame:
        ...
        boundary_block = pd.read_sql_query(
            boundary_query, self.conn, params=(epoch,)
        ).iloc[0]["boundary_block"]
        ...
        votes_df = pd.read_sql_query(
            vote_query, self.conn, params=(epoch, blocks_before)
        )
        ...
        """
        rewards_df = pd.read_sql_query(
            reward_query, self.conn, params=(epoch, blocks_before)
        )
    ...
    def optimize_allocation(
        self, df: pd.DataFrame, n_pools: int = 5, min_votes_filter: float = 0
    ...
    def analyze_epoch(self, epoch: int) -> Dict:
        ...
        results["actual_boundary"] = OptimizationResult(
            ...
            roi_percent=(
                (actual_return / self.voting_power * 100)
            ...
            try:
                ...
                results[f"{blocks_before}_blocks_before"] = {
                    "predicted": OptimizationResult(
                        ...
                        roi_percent=(
                            (pred_return / self.voting_power * 100)
                            if self.voting_power > 0
                            else 0
                        ),
                    ),
                    ...
                    "actual_roi_percent": (
                        (actual_return_from_pred / self.voting_power * 100)
                        if self.voting_power > 0
                        else 0
                    ),
                }
                ...
                logger.warning(
                    f"Could not analyze {blocks_before} blocks before for epoch {epoch}: {e}"
                )
            ...
            try:
                ...
                for blocks in [1, 20]:
                    ...
                    if epoch_results.get(key):
                        ...
                        return_gap_pct = (
                            (return_gap_usd / actual.expected_return_usd * 100)
                            if actual.expected_return_usd > 0
                            else 0
                        )

                        # Pool selection overlap
                        overlap = len(
                            set(actual.selected_gauges) & set(pred.selected_gauges)
                        )
                        ...
                        row[f"pred_{blocks}b_pools"] = ",".join(
                            pred.selected_gauges[:5]
                        )
                    else:
...
def main():
    ...
    output_path = (
        ...
        / "analysis"
        / "preboundary_optimization_results.csv"
    )
    ...
    for blocks in [1, 20]:
        ...
        if len(valid_data) > 0:
            ...
            print(
                f"Mean return gap: ${valid_data[f'gap_{blocks}b_usd'].mean():,.2f} ({valid_data[f'gap_{blocks}b_pct'].mean():.2f}%)"
            )
            print(
                f"Median return gap: ${valid_data[f'gap_{blocks}b_usd'].median():,.2f} ({valid_data[f'gap_{blocks}b_pct'].median():.2f}%)"
            )
            print(
                f"Mean pool overlap: {valid_data[f'pool_overlap_{blocks}b'].mean():.1f} / 5 pools"
            )
            print(
                f"Perfect predictions (overlap=5): {(valid_data[f'pool_overlap_{blocks}b'] == 5).sum()} / {len(valid_data)}"
            )

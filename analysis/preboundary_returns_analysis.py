...
def get_gauge_data_at_boundary(
...
def get_gauge_data_at_offset(
    conn: sqlite3.Connection, epoch: int, blocks_before: int
) -> Dict[str, Tuple[float, float]]:
    """
...
def calculate_roi_and_select_top5(
...
def analyze_epoch(conn: sqlite3.Connection, epoch: int) -> None:
    ...
    optimal_returns, optimal_details = calculate_expected_returns(
        boundary_top5, YOUR_VOTING_POWER, boundary_data
    )
    pre1_returns, pre1_details = calculate_expected_returns(
        pre1_top5, YOUR_VOTING_POWER, boundary_data
    )
    pre20_returns, pre20_details = calculate_expected_returns(
        pre20_top5, YOUR_VOTING_POWER, boundary_data
    )

    # Display results
    table = Table(
        title=f"Epoch {epoch} - Optimization Comparison (Normalized Token Units)"
    )
    ...
    for i, (label, top5) in enumerate(
        ...
        for rank, (gauge, votes, rewards, roi) in enumerate(top5, 1):
            console.print(
                f"  {rank}. {gauge[:10]}... ROI: {roi:.8f} ({rewards:,.2f} / {votes:,.0f})"
            )
